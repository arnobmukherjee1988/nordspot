...
FEATURE_COLS: list[str] = [
    ...
    # Weather
    "temperature", "wind_speed", "irradiance",
...
# Split conformal prediction widens [q05, q95] by a data-driven correction ĉ
...
def calibrate(
    ...
    Algorithm (Angelopoulos & Bates 2021, §2):
      ...
      2. Find ĉ = ⌈(n+1)(1−α)⌉/n quantile of {s_1,…,s_n}.
      3. Calibrated interval: [q05 − ĉ,  q95 + ĉ].

    This gives a finite-sample marginal coverage guarantee:
        P(y_{n+1} ∈ [q̂05 − ĉ, q̂95 + ĉ]) ≥ 1 − α
    ...
    Returns:
        ĉ — the correction in EUR/MWh (also saved to lgbm_conformal.pkl).
    """
    # Align on shared non-NaN index
    idx = (
        actuals.dropna().index
        .intersection(q05_preds.dropna().index)
...
def predict(df: pd.DataFrame, apply_conformal: bool = True) -> pd.DataFrame:
    ...
    Args:
        ...
        apply_conformal:  If True (default) and lgbm_conformal.pkl exists, the
                          q05/q95 interval is widened by ĉ to achieve
                          TARGET_COVERAGE marginal coverage.  Pass False when
                          predicting *for* calibration — otherwise the old
                          correction is baked into the inputs and ĉ collapses
