...
def _compute_metrics(
    ...
    for model, df in forecasts.items():
        ...
        results["crps"][model]           = _crps_quantile(y[mask], q05[mask], q50[mask], q95[mask])
        results["mae"][model]            = float(np.mean(np.abs(y[mask] - q50[mask])))
        results["coverage"][model]       = float(np.mean((y[mask] >= q05[mask]) & (y[mask] <= q95[mask])))
...
def walk_forward(
    ...
    *,
    ...
    while cursor + timedelta(days=test_days) <= end:
        ...
        if verbose:
            print(f"Fold {fold+1}: train {train_start.date()} -> {train_end.date()} "
    ...
    reliability_df = pd.concat(all_rel).groupby(["model", "nominal"])["observed"].mean().reset_index()

    return EvalResult(folds=fold_results, summary=summary_df, reliability=reliability_df)
...
if __name__ == "__main__":
    ...
    fake = pd.DataFrame({col: rng.standard_normal(n) for col in FEATURE_COLS}, index=idx)
    fake.index.name = "valid_time"
    fake["price"] = 50 + 10 * fake["hour_sin"] + 5 * fake["temperature"] + rng.standard_normal(n) * 15
