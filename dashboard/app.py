...
def load_features_range(from_str: str, to_str: str) -> pd.DataFrame:
    ...
    end = datetime.fromisoformat(to_str).replace(tzinfo=timezone.utc) + timedelta(
        days=1
    )
...
def load_forecasts_as_of(as_of_iso: str, from_iso: str, to_iso: str):
    ...
    end = datetime.fromisoformat(to_iso).replace(tzinfo=timezone.utc) + timedelta(
        days=1
    )
    all_ids = [
        SERIES["lgbm_q05"],
...
def models_available() -> bool:
    return Path("model/lgbm_q50.pkl").exists() and Path("model/lear_h00.pkl").exists()
...
stored_lgbm = (
    _stored_lgbm_full[_stored_lgbm_full.index >= _cutoff]
    if _stored_lgbm_full is not None
    else None
)
stored_lear = (
    _stored_lear_full[_stored_lear_full.index >= _cutoff]
    if _stored_lear_full is not None
    else None
)
...
with tab_fc:
    ...
    if fc_lgbm_src is not None:
        ...
        fc_df = (
            available.tail(24)
            if len(available) >= 24
            else (available if len(available) else None)
        )
    ...
    if fc_lgbm_src is not None:
        ...
        fig.add_trace(
            _band_trace(view.index, view["lgbm_q05"], view["lgbm_q95"], "90% band")
        )
        fig.add_trace(
            go.Scatter(
                x=view.index,
    ...
    if has_stored:
        ...
        if kt_df is not None:
            ...
            if not lgbm_kt.empty:
                kt_summary = lgbm_kt.groupby("knowledge_time")["valid_time"].agg(
                    ["min", "max", "count"]
                )
                ...
                kt_summary["First valid_time"] = kt_summary[
                    "First valid_time"
                ].dt.strftime("%Y-%m-%d %H:%M")
                kt_summary["Last valid_time"] = kt_summary[
                    "Last valid_time"
                ].dt.strftime("%Y-%m-%d %H:%M")
...
with tab_perf:
    ...
    with c5:
        ...
        metric_card(
            "Spike MAE", spike_val, f"n={m.get('n_spikes',0)} hours >100 EUR/MWh", RED
        )
    ...
    with col_l:
        ...
        fig = go.Figure(
            go.Bar(x=mae_hour.index, y=mae_hour.values, marker_color=colors)
        )
        fig.update_layout(
            ...
            cmp = pd.DataFrame(
                {
                    ...
                    "LightGBM": [
                        ...
                        m_all["lgbm"]["rmse"],
                        m_all["lgbm"]["mape"],
                        m_all["lgbm"]["coverage_q5_q95"],
                    ],
                    "LEAR": [
                        m_all["lear"]["mae"],
                        m_all["lear"]["rmse"],
                        m_all["lear"]["mape"],
                        m_all["lear"]["coverage_q5_q95"],
                    ],
                }
            ).set_index("Metric")
...
with tab_bt:
    ...
    with c2:
        ...
        metric_card(
            "Hours with forecast", f"{fc_count:,}", f"{100*has_fc.mean():.0f}% coverage"
        )
    with c3:
        mae_val = float(abs_err[has_fc].mean()) if has_fc.any() else None
        mae_color = GREEN if (mae_val or 99) < 20 else AMBER
        metric_card(
            "MAE (live)", f"{mae_val:.2f}" if mae_val else "-", "EUR/MWh", mae_color
        )
    with c4:
        if has_fc.any():
            cov = float(
                (
                    (actual[has_fc] >= p05[has_fc]) & (actual[has_fc] <= p95[has_fc])
                ).mean()
                * 100
            )
            metric_card(
                "PI Coverage", f"{cov:.1f}%", "q5-q95", GREEN if cov >= 85 else AMBER
            )
        else:
    ...
    if has_fc.any():
        ...
        fig.add_trace(
            _band_trace(
                fc_idx,
                bt_lgbm.loc[fc_idx, "lgbm_q05"],
                bt_lgbm.loc[fc_idx, "lgbm_q95"],
                "90% band",
            )
        )
        fig.add_trace(
            go.Scatter(
                x=fc_idx,
                y=bt_lgbm.loc[fc_idx, "lgbm_q50"],
                name="LightGBM p50 (live)",
    ...
    if bt_mode == "As-of query (end_known)":
        ...
        if as_of_lgbm is not None:
            ...
            mae_stored = (
                float(
                    np.abs(actual.reindex(as_of_lgbm.index) - as_of_lgbm["lgbm_q50"])
                    .dropna()
                    .mean()
                )
                if n_stored
            ...
            with cb:
                metric_card(
                    ...
                    "EUR/MWh",
                    GREEN if (mae_stored or 99) < 20 else AMBER,
                )
            with cc:
                delta = (
                    ((mae_stored or 0) - (mae_val or 0))
                    if (mae_stored and mae_val)
                    else None
                )
            ...
            fig_aof.add_trace(
                go.Scatter(
                    x=aof_valid.index,
                    y=aof_valid,
                    name=f"As-of {as_of_date} (TimeDB)",
                    line=dict(color=BLUE, width=2),
                )
            )
            apply_layout(fig_aof, f"As-of query: forecasts known before {as_of_date}")
        ...
        if da_lgbm is not None and da_lgbm.notna().sum() > 0:
            ...
            mae_da = float(
                np.abs(actual.reindex(da_lgbm.index) - da_lgbm).dropna().mean()
            )

            ca, cb, cc = st.columns(3)
            with ca:
                metric_card(
                    "Gate-closure hours", f"{n_da:,}", "available at noon D-1", BLUE
                )
            with cb:
                metric_card(
                    "MAE (gate-closure)",
            ...
            with cc:
                ...
                metric_card(
                    ...
                    f"{delta:+.2f}" if delta is not None else "-",
                    "gate-closure − live",
                    RED if (delta or 0) > 0 else GREEN,
                )
            ...
            fig_da.add_trace(
                go.Scatter(
                    x=da_valid.index,
                    y=da_valid,
                    name="Gate-closure forecast (TimeDB)",
                    line=dict(color=BLUE, width=2),
                )
            )
            if da_lear is not None:
                ...
                fig_da.add_trace(
                    go.Scatter(
                        x=da_lear_valid.index,
                        y=da_lear_valid,
                        name="LEAR gate-closure",
                        line=dict(color=AMBER, width=1.5, dash="dash"),
                    )
                )
            apply_layout(
                fig_da, "Gate-closure backtest - read_relative(days_ahead=1, noon D-1)"
            )
...
with tab_diag:
    ...
    if has_prices:
        ...
        if has_models and lgbm_pred is not None and n_spikes > 0:
            ...
            with c2:
                metric_card(
                    "Spike MAE - LightGBM", f"{spike_err.mean():.2f}", "EUR/MWh", RED
                )
            with c3:
        ...
        if n_spikes > 0:
            ...
            sp_fig.add_trace(
                go.Scatter(
                    x=df[spike_mask].index,
                    y=df.loc[spike_mask, "price"],
                    mode="markers",
                    name="Spike",
...
with tab_data:
    ...
    with col1:
        de_from = st.date_input(
            "From", value=date.today() - timedelta(days=7), key="de_from"
        )
    with col2:
    ...
    if dataset == "Prices":
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            metric_card("Mean", f"{p.mean():.2f}", "EUR/MWh")
        with c2:
            metric_card("Max", f"{p.max():.2f}", "EUR/MWh", RED)
        with c3:
            metric_card("Min", f"{p.min():.2f}", "EUR/MWh", GREEN)
        with c4:
            metric_card("Std", f"{p.std():.2f}", "EUR/MWh", GRAY)
        fig = go.Figure(
            go.Scatter(
                x=p.index,
                y=p,
        ...
        with tab_t:
            ...
            apply_layout(fig, "Temperature - Stockholm", "degC")
            st.plotly_chart(fig, use_container_width=True)
        with tab_s:
            fig = go.Figure(
                go.Scatter(
            ...
            apply_layout(fig, "Shortwave Solar Irradiance", "W/m^2")
            st.plotly_chart(fig, use_container_width=True)
...
- **Open-Meteo** - historical and forecast weather for Stockholm (59.33degN, 18.07degE)
